...
{
	canCallbacks.push({ addr: sourceaddress, port: sourceport, sub: subchannel, callback: callback, cb: function(data){
...
for (var i in conf.subchannels)
{
	...
	if (subchannel.type == "swt") {
        	registerCANCallBack(function(data, sub){
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('GetStat', function (data) {
		...
		for (var i in subchannels) {
			...
			if (subchannel.type == "swt") {
				clientChannels.push({'fkt': 'sw', 'dev': subchannel.id, 'val': subchannel.currentvalue});
			}
			if (subchannel.type == "sld") {
				clientChannels.push({'fkt': 'sl', 'dev': subchannel.id, 'val': subchannel.currentvalue});
			}
		}
	...
	socket.on('SetValue', function (data) {
		...
		if (subchannel.lapaddr < 0 ) {
			...
			subchannel.device.mpdclient.sendCommand(cmd(subchannel.action, mpdarg), function(err, msg) {
                               	if (err)
				{
